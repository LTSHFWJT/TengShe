...
func (message *HTTPMessage) SendMessage() {
	frame := drainHTTPFrame(message)
	if sender := SenderForConn(message.RawMessage.Conn); sender != nil {
		_ = sender.SendFrame(LaneForMessageType(message.RawMessage.MessageType), frame)
		return
	}

	_ = writeFull(message.RawMessage.Conn, frame)
}

func drainHTTPFrame(message *HTTPMessage) []byte {
	finalBuffer := make([]byte, 0, len(message.HTTPHeader)+len(message.HeaderBuffer)+len(message.DataBuffer))
	finalBuffer = append(finalBuffer, message.HTTPHeader...)
