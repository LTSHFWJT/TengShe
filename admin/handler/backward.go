...
func (backward *Backward) start(mgr *manager.Manager, topo *topology.Topology, uuid string) {
	...
	protocol.SendMessage(sMessage, header, seqMess, false)
	...
	share.ConfigureTCPConn(backwardConn)
	...
	result = <-mgr.BackwardManager.ResultChan
	if !result.OK {
		share.CloseQuietly(backwardConn)
		return
	}
	...
	go func() {
		for {
			if data, ok := <-dataChan; ok {
				if err := share.WriteFull(backwardConn, data); err != nil {
					share.CloseQuietly(backwardConn)
					return
				}
			} else {
...
func StopBackward(mgr *manager.Manager, uuid, route string, choice int) {
	...
	if choice == 0 {
		...
		protocol.SendMessage(sMessage, header, stopMess, false)
	} else {
		backwardTask := &manager.BackwardTask{
