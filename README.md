...
| `-s <secret>` | 通信密钥 |
| `-p tcp|icmp|dns` | 底层传输协议，默认 `tcp` |
| `--down raw|http|ws` | 下行消息封装，默认 `raw` |
...
| `--up raw|http|ws` | 上行消息封装，默认 `raw` |
| `--down raw|http|ws` | 下行消息封装，默认 `raw` |
| `--tls-enable` | TCP 链路启用 TLS |
| `--domain <name>` | TLS SNI / WS 域名 |
| `--cs utf-8|gbk` | shell 输出字符集 |
| `--socks5-proxy <ip:port>` | TCP 主动模式经 SOCKS5 代理连接 |
| `--http-proxy <ip:port>` | TCP 主动模式经 HTTP CONNECT 代理连接 |
...
| `connect <addr> [tcp|icmp|dns]` | 让当前节点主动连接下一跳 |
| `socks <port|ip:port> [user] [pass]` | 在 admin 本地启动 SOCKS5 |
| `stopsocks` | 停止 SOCKS5 |
...
```bash
script/integration_smoke.sh single
script/integration_smoke.sh multihop
script/integration_smoke.sh socks
script/integration_smoke.sh forward-backward
script/integration_smoke.sh file
```

更多验收步骤见 [selfdoc/integration_smoke.md](selfdoc/integration_smoke.md)。
