...
func (listen *Listen) normalListen(mgr *manager.Manager, options *initial.Options) {
	...
	defer share.CloseQuietly(listener)
	...
	for {
		...
		rMessage := protocol.NewDownMsg(conn, tsruntime.Component().Secret, protocol.ADMIN_UUID) //fake admin
		...
		if fHeader.MessageType == protocol.HI {
			...
			if mmess.Greeting == "Shhh..." && mmess.IsAdmin == 0 {
				...
				sLMessage := protocol.NewDownMsg(conn, tsruntime.Component().Secret, protocol.ADMIN_UUID) //fake admin
