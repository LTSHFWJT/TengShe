...
// backwardManager owns agent-side backward listener/session state.
// Lifecycle: BACKWARDTEST/BACKWARDSTART create a remote listener entry,
// BACKWARDSEQ/BACKWARDDATA use per-connection seq channels, and stop/offline
// paths close listeners, seq channels, and data channels.
type backwardManager struct {
	backwardSeqMap   map[uint64]string
	backwardMap      map[string]*backward
	BackwardMessChan chan interface{} // protocol messages dispatched by agent process

	TaskChan   chan *BackwardTask   // serialized manager commands
	ResultChan chan *backwardResult // command responses
	SeqReady   chan bool            // notification that a seq was delivered to a listener
}
...
func (manager *backwardManager) addConn(task *BackwardTask) {
	if _, ok := manager.backwardMap[task.RPort]; ok {
		...
		manager.backwardMap[task.RPort].backwardStatusMap[task.Seq].dataChan = make(chan []byte, backwardDataChanSize)
		manager.ResultChan <- &backwardResult{OK: true}
	} else {
		manager.ResultChan <- &backwardResult{OK: false}
	}
...
func (manager *backwardManager) getDataChan(task *BackwardTask) {
	if _, ok := manager.backwardMap[task.RPort]; ok {
		if status, ok := manager.backwardMap[task.RPort].backwardStatusMap[task.Seq]; ok && status != nil {
			manager.ResultChan <- &backwardResult{
				OK:       true,
...
func (manager *backwardManager) getDatachanWithoutUUID(task *BackwardTask) {
	...
	if bw, ok := manager.backwardMap[rPort]; ok {
		status, ok := bw.backwardStatusMap[task.Seq]
		if !ok || status == nil {
			manager.ResultChan <- &backwardResult{OK: false}
			return
		}
...
func (manager *backwardManager) closeTCP(task *BackwardTask) {
	...
	bw, ok := manager.backwardMap[rPort]
	if !ok {
	...
	delete(bw.backwardStatusMap, task.Seq)
	delete(manager.backwardSeqMap, task.Seq)

}

func (manager *backwardManager) closeSingle(task *BackwardTask) {
	bw, ok := manager.backwardMap[task.RPort]
	if !ok {
		manager.ResultChan <- &backwardResult{OK: true}
		return
	}

	share.CloseQuietly(bw.listener)
	share.CloseUint64ChanQuietly(bw.seqChan)
